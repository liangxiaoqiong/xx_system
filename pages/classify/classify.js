...
Page({
  ...
  getClassify: function () {
    ...
    api.wxRequest({
      ...
      var barTitle = wx.getStorageSync('storeData').store_name || ''
      ...
      if (+getApp().globalData.psWay === 1) {
        ...
        mimi_data = res.data.decoration_data.mini_wm_data
      } else {
        // 堂食
        banner_ctrl = res.data.decoration_data.mini_ts_ad_ctrl
        mimi_data = res.data.decoration_data.mini_ts_data
      }
      var bannerData = {}
      var imgList = []
      mimi_data.img_list.forEach(function (value) {
        imgList.push({
          action: value.action,
          action_data: value.action_data,
          img_url: value.img_data ? value.img_data.img_url : value.img_url,
          url: value.img_data ? value.img_data.url : value.url,
          swiper_bg_color: value.swiper_bg_color ? value.swiper_bg_color : "#fe3434",
          swiper_bg_style: value.swiper_bg_style ? value.swiper_bg_style : "0",
        })
      })
      bannerData.img_list = imgList
      bannerData.img_module0_height = mimi_data.img_module0_height ? mimi_data.img_module0_height : '270'
      bannerData.selected_img_module = mimi_data.selected_img_module ? mimi_data.selected_img_module : '0'
      if (typeof mimi_data.img_module !== 'undefined') {
        bannerData.img_module = mimi_data.img_module
      } else {
        bannerData.img_module = [
          {
            btn_color: "#fe3434",//按钮颜色
            btn_position: "1",//0-按钮居左，1-按钮居中，2-按钮居右
            btn_style: "0",//0-长方形，1-正方形，2-圆形
            img_height: "270",//轮播高度
            img_height2: "36%",//轮播高度
            swiper_autoplay: "1800",//轮播切换间隔*/
            swiper_style: "0",//0-平铺式，1-悬浮式
          }
        ]
      }
      if (+banner_ctrl === 1 && bannerData.img_list.length > 0) {
        banner_height = wx.getSystemInfoSync().windowWidth / (750 / +bannerData.img_module[0].img_height)
      }
      ...
      self.adBanner = self.selectComponent('#adBanner')
      self.adBanner.initLoad(self.data.bannerData)
      self.resetSearch()
    })
  },
  //切换左侧分类
